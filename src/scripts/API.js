...
class API {
     ...
     * @returns A promise resolving when the API call is done
     */
    static execute_logged(path, method = API.METHOD.GET, body = {}, type = this.TYPE.JSON, headers = []) {
        return new Promise((resolve, reject) => {
            if (!User.CurrentUser || !User.CurrentUser.token) {
                reject({status: -1, message: "Error: User not defined or logged in"});
                return;
            }
            ...
            reqHeaders[API.AuthorizationHeader] = User.CurrentUser.token;
